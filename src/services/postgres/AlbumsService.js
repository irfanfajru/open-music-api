...
const songsModel = require('../../utils/songsModel');
...
class AlbumsService {
  ...
  async getAlbumById(id) {
    ...
    const result = {
      ...resultAlbum.rows.map(albumModel)[0],
      songs: resultSongs.rows.map(songsModel),
    };
    return result;
  }
  ...
  async editCoverAlbum(albumId, cover) {
    ...
    const result = await this._pool.query(query);
    if (!result.rows.length) {
      throw new NotFoundError('Album cover gagal diperbarui,Id tidak ditemukan');
    }
  }
}
